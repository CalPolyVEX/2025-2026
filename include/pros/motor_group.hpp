...
class MotorGroup : public virtual AbstractMotor {
  ...
  public:
     ...
     * EDOM - The motor group is empty
     *
     * \return A vector containing the following for each motor: 1 if the motor's current limit is being exceeded and 0
     * if the current limit is not exceeded, or PROS_ERR if the operation failed, setting errno.
     *
     * \b Example
     * \code
     * void opcontrol() {
